  ...
  function(z){
    ...
    for (h in seq_along(sample_size)){
      ...
      for (j in seq_along(dates_ss)){
          ...
          while(!valid){
            ...
            resamp_dates <- vapply(
              ...
              function(k){
                runif(
                  1,
                  min(cal_dates$grids[[k]][,1]),
                  max(cal_dates$grids[[k]][,1])
                )
              },
              numeric(1)
            )
        ...
        res_list[[row_id]] <- data.frame(
          Estimate = means[1],
          upperCI = means[2],
          lowerCI = means[3],
          start_date = dates_subset$start_date[1],
          r = dates_subset$r[1],
          Sd = dates_subset$Sd[1],
          ESS = dates_ss[j],
