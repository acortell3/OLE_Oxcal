...
for (i in 1:length(start_date)){
	for (j in 1:length(r)){
		for (k in 1:length(C14_errors)){
			...
			write.table(YearsBP,file = paste0("../Simu_data/Uncal_YearsBP_sim_hol_",i,"_r_",r[j],"_error_",C14_errors[k],".txt"), sep = "\t", row.names = F) ## It purposedly overwrites the above
		}
	}
}
